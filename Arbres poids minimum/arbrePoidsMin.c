...
graphe * algoKruskal(graphePondere *Gp){
  ...
  O = triAretes(Gp); /* O[i] est l'index de la i-�me ar�te par ordre croissant de poids; 
    // les extremites de la i-�me aretes sont donc Gp->I[O[i]] et Gp->T[O[i]] */

  /* A completer ici */
  int n = Gp->nsom ;

/*
  for(int i = 0 ; i < Gp->narc ; i++)
    printf("%d %d %f\n", Gp->I[O[i]], Gp->T[O[i]], Gp->poids[O[i]]) ;
  ...
  */
  ...
  while(k < n-1){
    ...
    if(Z[y] == FAUX){
      ...
      k++ ; 
      //printf("Ajoute : k=%d O[i]=%d %d %d %f %f\n", k, O[i], x, y, Gp->poids[O[i]], poidsArbre) ;
    }
