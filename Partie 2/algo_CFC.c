...
booleen * CFC(graphe* G, int x){
  booleen *X;
  booleen *Y;       /* tableau booleens pour stocker l'exploration */
  booleen *Z;   
  
  X = explorationLargeur(G, x) ;
  graphe* Gsym = symEfficace(G); 
  Y = explorationLargeur(Gsym, x) ;
  Z = (booleen*) calloc(G->nsom, sizeof(booleen));

  for(int i=0 ; i < G->nsom ; i++){
    if(X[i] == VRAI && Y[i] == VRAI) Z[i] = VRAI ;
  }
